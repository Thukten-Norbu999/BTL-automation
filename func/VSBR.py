    ...
    for dt in det:
        if len(dt)>1 and len(dt) < 140:
            try:
                ...
                subNo, subSts, adSts, cOut, cIn, supSer = dt[18].removeprefix("Subscribernumber="),dt[28].removeprefix("Subscriberstatus="),dt[29].removeprefix("Additionalstatus="), "\n".join(call_out(dt)[0]), "\n".join(call_in(dt,c_Out)[0]), dt[c_In+3]
